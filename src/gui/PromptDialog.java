...
public class PromptDialog extends JDialog {
	...
	private static final long serialVersionUID = 7669170999453729047L;
	...
	/**
	 * Builds a prompt dialog, which includes two labeled text fields in which a
	 * user can enter his desired length and difficulty. The user can push one
	 * of three buttons: start, load or cancel. When start is clicked, the
	 * inserted values for length (returnArray[0]) and difficulty
	 * (returnArray[1]), as well as the error code 0 (returnArray[2]) are
	 * written into the returnArray, which functions kind of as a return object.
	 * If load is clicked, a file browser opens in which the user can choose a
	 * file, which gets loaded. If the loading process was successful, -2 is
	 * written into the returnArray[2] indicating that the dialog took care of
	 * the further process and the dialog is closed. The third option is to
	 * click the cancel button. When doing that, -1s are written into the
	 * returnArray[2] indicating that the method returned an error value. The
	 * program should stop after detecting these.
	 * 
	 * @param suggestLength
	 * @param suggestDifficulty
	 * @param returnArray
	 */
	public PromptDialog(String suggestLength, String suggestDifficulty, int[] returnArray) {
		...
		JLabel noteRow1 = new JLabel(
				"Bitte geben Sie eine Quadratzahl als Länge ein, welche nicht größer " + "als 16 ist");
	 ...
	 * 
	 * @param suggestLength
	 *            The length which should be corrected
	 * @return The corrected length as String. 9 if empty, null if illicit and
	 *         the suggested length otherwise.
	 */
			...
			try {
				...
				if (Math.sqrt(number) % 1 != 0 || number > 16) {
					JOptionPane.showMessageDialog(null, "Unerlaubte Länge eingegeben. Sie ist keine "
							+ "Quadratzahl und/oder zu groß:   " + number);
				...
				JOptionPane.showMessageDialog(null,
						"Unerlaubte Länge eingegeben. Länge ist keine Zahl:   " + suggestLength);
				returnString = null;
			}
		}
		return returnString;
	 ...
	 * 
	 * @param suggestDifficulty
	 *            The difficulty which should be corrected
	 * @return The corrected difficulty as String. 5 if empty, null if illicit
	 *         and the suggested difficulty otherwise.
	 */
		...
		if (suggestDifficulty.equals("")) {
			...
			try {
				...
				if (number > 10 || number < 1) {
					JOptionPane.showMessageDialog(null, "Unerlaubte Schwierigkeit eingegeben. Schwierigkeit ist nicht "
							+ "innerhalb des erlaubten Bereichs (1-10):   " + number + ".");
					returnString = null; // input illicit: out of bounds
				}
			} catch (NumberFormatException ex) {
				JOptionPane.showMessageDialog(null,
						"Unerlaubte Schwierigkeit eingegeben. Schwierigkeit ist keine Zahl:   " + suggestDifficulty);
